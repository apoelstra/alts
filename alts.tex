...
functions such as \texttt{SHA256d} which are easy to calculate but have
unpredictable outputs, are called \emph{one-way functions}. However, \texttt{SHA256d} is
...
Further, the total value of inputs must be greater than or equal to
the total value of the outputs (input values are defined as the values
...
a natural way to weight each block, so that for every path in the blockchain
...
than ten minutes for long periods of time. This specter has motivated some altcoin
...
a ``limping blockchain'' as they cycle through them.
...
below) and could not achieve any consensus. Since becoming closed-source\footnote{In
March 2015, I was contacted by a NXT developer who informed me that NXT is now available
under the open-source MIT license.} while spamming
...
Dan Boneh also offers excellent courses in cryptography online, for free. They
can be accessed at the following URL's.
\begin{itemize}
\item \url{https://www.coursera.org/course/crypto}
\item \url{https://www.coursera.org/course/crypto2}
\end{itemize}
